---
id: PROC-TAX-CONF-001
title: "Cyprus Non-Domicile (Non-Dom) Status Application"
category: tax
subcategory: tax-residency
status: draft
confidence: medium
source_type: mixed
sources:
  - url: "https://www.globalcitizensolutions.com/cyprus-non-domiciled-tax-residence/"
    title: "Cyprus Non-Dom Tax Residence: A Full 2026 Guide — Global Citizen Solutions"
    accessed: "2026-04-09"
  - url: "https://luma.cy/cyprus-non-dom-tax-status/"
    title: "Understanding Non-Domicile Tax Status in Cyprus — Luma"
    accessed: "2026-04-09"
  - url: "https://gk-lawfirm.com/cyprus-tax-residency/"
    title: "Cyprus tax residency and non domicile rules explained 2026 — GK Law Firm"
    accessed: "2026-04-09"
  - url: "https://www.cyprustaxlife.com/learn/non-dom"
    title: "Cyprus Non-Dom Status 2026: 0% Dividend Tax for 17 Years — Cyprus Tax Life"
    accessed: "2026-04-09"
  - url: "https://www.ktc.com.cy/news/cyprus-non-dom-tax-status-guide/"
    title: "Cyprus Non-Dom Tax Status 2026 — KTC"
    accessed: "2026-04-09"
  - url: "https://www.easycorporate.com.cy/blog/special-defence-contribution"
    title: "Special Defence Contribution (SDC): Complete Guide Updated 2026 — Easy Corporate"
    accessed: "2026-04-09"
  - url: "https://assets.kpmg.com/content/dam/kpmg/cy/pdf/2024/cyprus-tax-residency-and-non-dom-rules.pdf"
    title: "Cyprus Tax Residency and Non-Dom Rules — KPMG Cyprus"
    accessed: "2026-04-09"
last_validated: ""
validated_by: ""
created: "2026-04-09"
updated: "2026-04-13"
review_due: "2026-10-13"
related_processes:
  - "PROC-TAX-PROC-001 — 60-Day Tax Residency Rule"
  - "PROC-IMM-001 — Yellow Slip Application (MEU1)"
  - "PROC-BIZ-001 — Company Formation"
related_partners:
  - tax-specialist
  - accountant
tags:
  - tax
  - non-dom
  - tax-residency-structuring
  - cyprus-wide
  - eu-nationals
  - uk-nationals
  - third-country-nationals
  - business-owners
---

## Summary

Cyprus Non-Domicile (Non-Dom) status is a tax classification available to individuals who are Cyprus tax residents but are not legally considered domiciled in Cyprus. Non-doms are fully exempt from the Special Defence Contribution (SDC) on worldwide dividend and passive interest income, as well as rental income — for up to 17 years. This makes Cyprus one of the most attractive jurisdictions in Europe for business owners, investors, and high-net-worth individuals who receive dividends or investment income. Non-dom status is not a separate application in isolation — it is obtained by filing a declaration with the Cyprus Tax Department confirming that you qualify, alongside your annual tax return.

---

## Who This Applies To

To qualify for non-dom status, you must satisfy **both** of the following:

**1. Be a Cyprus tax resident**
- Either under the 183-day rule (spending more than 183 days in Cyprus per calendar year)
- Or under the 60-day rule (see PROC-TAX-PROC-001)

**2. Not be domiciled in Cyprus under Cyprus law**

You are considered **not domiciled** in Cyprus if you meet one of the following:
- You were not born to a Cypriot-domiciled parent (i.e. you have a "domicile of origin" outside Cyprus)
- You have not been a Cyprus tax resident for 17 or more of the last 20 years (this is the automatic disqualification threshold — you lose non-dom status after 17 years of Cyprus tax residency)

**In practice:** The vast majority of relocators from the UK, EU, US, and other countries will qualify as non-domiciled. You would only be considered domiciled in Cyprus if you were born there to Cypriot parents or have lived there for most of your adult life.

**Duration:** Non-dom status lasts for up to 17 years from when you first became a Cyprus tax resident. After 17 years of Cyprus tax residency, you are deemed domiciled and lose the SDC exemption.

---

## What Non-Dom Status Provides

| Income Type | Non-Dom Treatment | Standard Cyprus Treatment |
|-------------|------------------|--------------------------|
| Dividend income (Cyprus & foreign companies) | 0% SDC | 5% SDC (as of 2026) |
| Passive interest income | 0% SDC | 17% SDC |
| Rental income | 0% SDC | 3% SDC (on 75% of gross rent) |
| Capital gains on securities | Exempt | Exempt (same for all) |
| Employment income | Normal income tax rates apply | Same |

**GHS (General Healthcare System) contributions still apply** for non-doms at 2.65% on dividend and interest income, capped at €180,000. This is separate from SDC and is not exempted by non-dom status.

**2026 SDC reform note:** SDC on dividends was reduced from 17% to 5% for non-qualifying individuals as of January 2026. SDC on rental income was abolished entirely. Non-doms retain full exemption from SDC on dividends and interest.

---

## What You Will Need

### Documents Required

Non-dom status is established through annual declarations filed with the Cyprus Tax Department. There is no separate "non-dom application" — you declare your status as part of the tax filing process.

| Document | Purpose | Notes |
|----------|---------|-------|
| TD.38 form (or equivalent SDC declaration) | Declaration of non-dom status to the Tax Department | Filed alongside or as part of annual tax return |
| Evidence of Cyprus tax residency | Proves you meet the tax residency condition | Travel records, rental agreement, TIC registration |
| Evidence of domicile of origin | Proves you are not domiciled in Cyprus | Typically established by nationality and birth — rarely requires separate documentation |
| TIC (Tax Identification Code) | Required for all Cyprus tax filings | Must be obtained before filing |

---

### Fees

| Fee | Amount | Notes |
|-----|--------|-------|
| Non-dom declaration (TD.38) | Free | No government fee |
| Annual tax return filing | Free (self-filed) | Accountant fees apply if using a professional |
| Tax specialist fees | Variable | Highly recommended for first-year setup; €500–€2,000 typical for initial structuring |

**Proposed 2026 change (not yet enacted):** A flat annual fee of €5,000–€10,000 has been proposed for new non-dom applicants. This had not been formally enacted as of April 2026. Monitor for legislative updates.

---

### Where to Apply

Non-dom status is declared to the **Cyprus Tax Department** as part of the annual tax return process.

- **Online:** Via the Taxisnet portal at [https://www.taxisnet.gov.cy](https://www.taxisnet.gov.cy)
- **In person:** At the local Tax Department district office
- **Via a tax specialist or accountant:** Recommended for first-time applicants

---

## Step-by-Step Process

1. **Establish Cyprus tax residency first.** Non-dom status is only available to Cyprus tax residents. Complete PROC-TAX-PROC-001 (60-day rule) or ensure you meet the 183-day threshold before proceeding.

2. **Obtain a Tax Identification Code (TIC).** If not already registered, apply at the local Tax Department or via Taxisnet.

3. **Engage a Cyprus tax specialist.** The structuring of non-dom status — particularly the interaction with dividend income, GHS contributions, and any foreign tax obligations — requires professional advice. Do not attempt to self-file for the first year without specialist guidance.

4. **Confirm your domicile of origin.** In practice this is straightforward for most relocators — if you were not born to Cypriot parents and have not been Cyprus tax resident for 17+ years, you qualify. Your tax specialist will confirm this.

5. **File the non-dom declaration (TD.38 or equivalent).** This is filed with the Cyprus Tax Department alongside your annual tax return (TD.1). It declares that you qualify for non-dom status for that tax year.

6. **Notify banks and dividend-paying companies of your non-dom status.** To prevent SDC being withheld at source on dividend payments from Cyprus companies, notify the paying company or bank of your non-dom status with supporting documentation.

7. **File annually.** Non-dom status must be declared each tax year. It is not a one-time application. Your tax specialist should include this in your annual filing.

---

### Timeline

- Non-dom status takes effect from the date you became a Cyprus tax resident — it is not prospective only
- The annual declaration (TD.38) is processed as part of the tax return — no separate waiting period
- Tax residency certificates confirming non-dom status can be requested from the Tax Department and are typically issued within 2–4 weeks

---

## Field Notes

> **These notes reflect field intelligence from practitioner sources. They are not official guidance and should be verified with a qualified Cyprus tax specialist.**

**FN-001 — Non-dom status and the 17-year clock**
The 17-year non-dom period begins from your first year of Cyprus tax residency — not from when you first claimed non-dom status. If you have been a Cyprus tax resident for several years without claiming non-dom benefits, you may have already used some of your 17 years. A tax specialist should calculate your remaining non-dom eligibility from your actual first year of tax residency.
*Source: Multiple practitioner guides, April 2026.*

**FN-002 — GHS contributions are frequently overlooked**
Non-doms are exempt from SDC on dividends but must still pay GHS contributions at 2.65% on dividend and interest income, capped at €180,000 per year. This is commonly misunderstood. The maximum GHS contribution on passive income is therefore €4,770/year. Factor this into planning.
*Source: Multiple practitioner guides, April 2026.*

**FN-003 — Proposed flat annual fee not yet law (April 2026)**
A proposal to introduce a €5,000–€10,000 annual flat fee for non-dom status was circulating as of April 2026. It had not been formally enacted as of the date of this research. This should be monitored closely as it would materially change the economics of non-dom status for lower-income applicants.
*Source: Practitioner guides, April 2026. Status: proposed only.*

**FN-004 — 2026 SDC reform preserves non-dom advantage**
The January 2026 reform reduced SDC on dividends from 17% to 5% for non-qualifying individuals. Non-doms retain 0% SDC. The reform narrows (but does not eliminate) the advantage of non-dom status for dividend income.
*Source: Easy Corporate SDC guide, April 2026.*

---

## Common Issues and How to Handle Them

**Issue:** Applicant has been Cyprus tax resident for years but never formally claimed non-dom status.
**Resolution:** Non-dom status can typically be claimed retroactively for open tax years. Engage a tax specialist to review your filing history and file amended returns if appropriate.

**Issue:** SDC is being withheld at source on dividend payments from a Cyprus company.
**Resolution:** Provide the Cyprus company or its accountant with your non-dom declaration (TD.38) and TIC. They should then cease withholding SDC on your distributions.

**Issue:** Applicant is approaching the 17-year threshold and is unsure when the clock started.
**Resolution:** The 17-year count begins from the first year you were a Cyprus tax resident — even if you did not live there full-time. A tax specialist should review your residency history and calculate the remaining period.

**Issue:** Foreign tax authority disputes the tax benefit arising from Cyprus non-dom status.
**Resolution:** Obtain a Cyprus Tax Residency Certificate and non-dom declaration from the Tax Department. Ensure you genuinely meet all Cyprus tax residency conditions. Seek cross-border tax advice if operating in multiple jurisdictions.

---

## Related Processes

- **PROC-TAX-PROC-001 — 60-Day Tax Residency Rule:** Tax residency is the prerequisite for non-dom status. These processes always run together.
- **PROC-IMM-001 — Yellow Slip Application (MEU1):** EU nationals will typically obtain the Yellow Slip alongside establishing tax residency.
- **PROC-BIZ-001 — Company Formation:** Many non-doms structure dividend income through a Cyprus company. Company formation and non-dom status are closely linked for business owners.

---

## Changelog

| Date | Change | Author |
|------|--------|--------|
| 2026-04-13 | Updated to new process document schema (review_due, section headings) | |
| 2026-04-09 | Initial draft created from secondary sources | |
